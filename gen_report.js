  ...
  sections: [
    ...
    // ===== MAIN CONTENT =====
    {
      ...
      children: [
        ...
        bullet("服务识别：内置 300+ 常见端口到服务名称的映射表，自动识别端口对应服务。"),
        ...
        ...[
          ...
          "    ├── utils.py       IP/端口解析，300+ 服务名映射，JSON/CSV 导出",
        ...
        new Table({
          ...
          rows: [
            ...
            new TableRow({ children: [cell("工具模块", 1800), codeCell("utils.py", 2200), cell("parse_ip_target/parse_port_range 解析，300+ 服务名映射，export_json/export_csv 导出", 5360)] }),
            ...
            new TableRow({ children: [cell("GUI 启动器", 1800), codeCell("gui.pyw", 2200), cell("双击启动，无命令行窗口。run_gui.vbs 备用 VBS 启动器", 5360)] }),
        ...
        p("启动方式：双击 gui.pyw（Windows 关联 pythonw.exe，无命令行窗口），或双击 run_gui.vbs（VBScript 后台启动）。"),
        ...
        bullet("--target：目标网段（支持单IP/CIDR/范围/域名），必填"),
        ...
        bullet("300+ 端口服务映射表大大提升了扫描结果的可读性，用户一眼就能知道开放端口对应的服务。"),
